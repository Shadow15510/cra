...
class AirSounding:
    ...
    def show(self, layers: tuple=()):
        ...
        idx = mpcalc.resample_nn_1d(
                self.data['press'],
                np.arange(100, 1000, 25) * units.hPa
            )
        ...
        if layers:
            skew.plot(layers[0], layers[1], 'o', color='darkorange', label='Couches auto-détectées')
    ...
    def auto_layers(self, layer_size: int=0, nb_layers: int=10, show: bool=False):
        ...
        for index, info in enumerate(self.__layers_analysis(layers)):
            title = f'Couche n°{index + 1}'
            print(f'{title}\n' + len(title) * '-')

            print('Limites')
            print(f'.. pression    : {press[index]} hPa – {press[index + 1]} hPa')
            print(f'.. température : {temp[index]}°C – {temp[index + 1]}°C')

            print('Informations')
            print(f'.. température potentielle                        : {get_evolution(info[0])}')
            print(f'.. température potentielle du thermomètre mouillé : {get_evolution(info[1])}')
            print(f'.. saturation base de la couche                   : {get_bool(info[2])}')
            print(f'.. saturation sommet de la couche                 : {get_bool(info[2])}')
